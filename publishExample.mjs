...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'convex-auth-publish-example-'));
...
shell.exec('git init');
shell.exec('git add .');
shell.exec('git commit -m "Published from https://github.com/get-convex/convex-auth"');
...
shell.exec(`find src -type f | xargs perl -i -pe 's#\\{import\\.meta\\.env\\.BASE_URL\\}#"/"#'`);
